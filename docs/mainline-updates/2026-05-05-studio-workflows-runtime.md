...
Finalization `758d1699f4742ef781d36d1f14753f23e9705dc7` was pushed and evaluated by GitHub Actions
run `30199620889` on pull request #3. The shared runtime audit step and the Aggregate
mainline-note reconciliation step both succeeded on the runner. The governance suite then reported
988 passed and 1 failed in 3077 seconds, comfortably inside the newly calibrated 120-minute job
timeout, so both delivery-surface calibrations behaved as intended.

The single failure is a real shared-layer defect, not an environment artifact.
`studio/tests/check-speckit-runtime.Tests.ps1` asserts that nine consumer-space files under
`learning/` and `projects/` exist before it exercises the R-G01 revert-sensitivity fixture. Those
directories are excluded by `.gitignore`, so the files are absent from every fresh clone. The
assertion therefore passes on any workstation that happens to hold local consumer state and fails
on any clean checkout, which is a false green wherever the suite has previously been accepted. It
also contradicts the Studio Constitution rule that `projects/` and `learning/` are consumer spaces
and must not be treated as the shared-layer acceptance surface.

This note and the dedicated 2026-07-26 Batch note return to Draft/Open together with their index
rows. This consumes the first of the two finalization re-entries permitted by plan Section 38. No
finding status changes on this demotion alone, R-E09 remains `IN_PROGRESS`, R-J03 remains `OPEN`,
and no merge, PR-thread resolution or post-merge action is authorized.

## Section 38 Closure Baseline Re-entry (2026-07-26)

Owner-authorized wave-3 plan Section 38 (`f428029467f3ba214ee6eef1eb6b4d5983f28aed`) records the
closure baseline after honesty demotion `4ee48a05c40acc10fb88d38902f412608c1c7566`: environment
establishment is the goal, slow gates move onto the Draft accounting-candidate tree before any
finalization, the finalization diff is machine-limited to note and index files, at most two
finalization re-entries are permitted, and push, merge and post-merge accounting are three
separate owner authorization nodes. This section supersedes the stale current-state statements
below while preserving their historical evidence.

The elevated fixture failure reproduced in a directly executed canonical pwsh 7.5.4 terminal with
a CP950 console, so it is a real defect, not a sandbox artifact. Ledger revision 10
(`3393d9bb5784d9a4e0a2812bde2efbc264b31446`) registers it as Medium R-A23. Repair
`f8d064c81b592e1c42966a68db6325f1685db089` gives `Invoke-RuntimeAuditFixture` an explicit UTF-8
process-output decoder with an old-fails/new-passes discriminating test, and the owner verified
both the previously failing bad-state test and the new test green in the same canonical terminal.
Revision 11 (`00424901aa315d708c88c763ca77f52db3b981e5`) records R-A23 as `COMPLETED`; the fold
is 96 `COMPLETED` / 1 `OPEN` / 0 `DECIDED` / 1 `IN_PROGRESS` / 35 `DISPOSITIONED` across 133
findings.

The same batch calibrates the delivery surface without touching any contract-pinned line: CI
coverage is limited to schedule and dispatch with the job timeout calibrated to 120 minutes
(`742a7fba7cbf088195211f0e35432c4734858b78`), and the README consumer-directory description now
discloses that `projects/` and `learning/` are untracked and absent from a public clone
(`b63dff89fda341c3d291e48a57403458d5033deb`). Both carry revert-sensitive Pester assertions, and
...
Report-export finalization `d0c75c4ed25f79fcecb74c387df00da103049b4c` passes staged-snapshot
validation after the explicitly approved CIM preflight. The elevated official suite discovers the
expected 986 tests in 27 files, but `check-speckit-runtime.Tests.ps1` produces an actual Pester
failure in `rejects additive stale phase path metadata and Claude authority guidance`.

The failing fixture invokes a child runtime audit that exits 1 with the expected ten governed
failures, but `Invoke-RuntimeAuditFixture` cannot parse the captured child output with
`ConvertFrom-Json`. The captured diagnostic contains corrupted non-ASCII text. This differs from
the previous restricted-sandbox run, where all 986 tests passed before report export, and requires
a focused encoding-boundary diagnosis before another complete suite. The suite was terminated
after the first conclusive Pester failure because the required zero-failure result was no longer
possible; it therefore supplies neither a complete count nor a new accepted NUnit report.

This Aggregate note and matching index row return immediately to Draft/Open. The failure does not
change any finding status or the Ready R6 Batch umbrella. R-E09 remains `IN_PROGRESS`, R-J03
remains `OPEN`, and no push, merge, workflow promotion, PR-thread resolution or post-merge action
is authorized.

## R6-A6 Report-Export Access Re-entry

Plan `af1cc007841cd31786d35cb53880e309c2584906` authorizes this documentation-only
finalization after honesty demotion `4c5fde387fae49309191fbaf0e6ddb579db7c53b`. An explicitly approved
read-only preflight returned a non-empty Windows 11 operating-system identity from
`Get-CimInstance Win32_OperatingSystem`, proving that the validation environment can supply the
runtime metadata required by Pester's NUnit report exporter.

This note and matching index row return to Ready/Closed subject to the exact-tree gates in plan
version 1.31.0. The unchanged official governance entrypoint must exit 0 after exactly 986 passed,
0 failed, 0 skipped, 0 inconclusive and 0 not run; it must write a new NUnit XML report with the
same aggregate counts. Persistent execution-policy scopes must match exactly before and after.
Runtime, nine-record finding history, Batch and Aggregate must then remain valid with 0 errors and
0 warnings, and historical evidence must remain 18 of 18 valid. Any deviation immediately restores
Draft/Open.
...
|---|---|---|---|
| `README.md` | `must_update` | `updated` | R6-A1 implementation `105a09cd02f7d8b4765e49859390908e55bd97d1` aligns current governance and authority truth, and README truthfulness `b63dff89fda341c3d291e48a57403458d5033deb` adds the consumer-directory disclosure; the R6-A1 Batch note and the 2026-07-26 delivery-calibration Batch note cover both. |
...
| `docs/README.md` | `must_update` | `updated` | R-A23 accounting `00424901aa315d708c88c763ca77f52db3b981e5` records revision 11 and fold 96/1/0/1/35; the 2026-07-26 delivery-calibration Batch note covers the ledger and index updates. |
