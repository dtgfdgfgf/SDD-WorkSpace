...
|------|-------|---------------|--------|---------|
| 2026-07-13 | [`r0-containment-and-source-cleanup`](./2026-07-13-r0-containment-and-source-cleanup.md) | `feature/wave-3-security-and-workflows` | Draft | Owner-approved R0 baseline: remove 396 obsolete or unlicensed tracked files plus local residue; add bounded MIT and third-party provenance; demote workflow catalog metadata; add staged-path privacy protection and workspace-scoped noreply identity. Runner authorization and server-side enforcement remain scheduled follow-ups. |
| 2026-07-12 | [`workflow-engine-completion-integrity`](./2026-07-12-workflow-engine-completion-integrity.md) | `feature/wave-3-security-and-workflows` | Ready | Fix workflow-engine false-completion/replay defects (verified claims C1-C5): agent steps require an artifact change (SHA-256 fingerprint + `-AcceptAgent` override) not mere existence; `completed_steps` skip on resume + idempotent setup-plan prevents overwrite; `extract` block populates switch subjects (readiness/eci). 4 new engine tests, new completion-integrity invariant. `sdd-pipeline` is now a real acceptance signal. |
| 2026-07-12 | [`agent-conformance-and-doc-drift`](./2026-07-12-agent-conformance-and-doc-drift.md) | `feature/wave-3-security-and-workflows` | Ready | Constitution conformance from verified analysis: remove specify-to-readiness (A1) and tasks-to-implement (A3) skip handoffs, remove clarify skip clause (A2), retighten specify to stop inventing requirements (A5), fix QA agent authority note (F2); re-seed Claude mirrors (fixes analyze-mirror drift F1). Doc drift: remove dangling features.txt refs (F4). No constitution/contract change. (F3 copilot phase line deferred: adapter-sync gate requires tri-adapter co-staging.) |
| 2026-07-12 | [`analyze-completion-gate`](./2026-07-12-analyze-completion-gate.md) | `feature/wave-3-security-and-workflows` | Ready | `setup-implement.ps1` now blocks when `/speckit.analyze` never ran (missing `analysis-checklist.md`) or is not signed off (`**Analysis Status**: PENDING`), closing the one ungated mandatory transition. Adds Analyze Gate marker to `checklist-template.md`, `ANALYZE_STATE` output, 2 negative-path tests, 1 scriptInvariant. Machine-closes verified claims B1-B3 and the tasks-to-implement skip (A3). |
