...
- **M8** — `new-project-worktree.ps1` created git worktrees but did not configure `core.hooksPath`. Worktrees do NOT automatically inherit `core.hooksPath` from the main repo when using a relative path: a relative `../../.githooks` resolves differently from inside the worktree, so commits from the worktree could silently bypass the workspace hooks.
...
- 142 → 166 tests, 0 failed, 0 skipped.
...
- `new-project-worktree.ps1` end-to-end test confirms `core.hooksPath` is set to a path matching `../*/.githooks` from inside the worktree.
