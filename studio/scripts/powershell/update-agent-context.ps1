...
Mirrors the behavior of scripts/bash/update-agent-context.sh:
 ...
 5. Multi-Agent Support (claude, gemini, copilot, cursor-agent, qwen, opencode, codex, windsurf, kilocode, auggie, roo, codebuddy, amp, shai, q, qodercli, kiro-cli, kiro, agy, bob, jules)
...
$STUDIO_ROOT = Find-StudioRoot -StartDir $REPO_ROOT
$TEMPLATE_FILE = $null
if ($STUDIO_ROOT) {
    $studioTemplate = Join-Path $STUDIO_ROOT 'templates/sdd-docs/agent-file-template.md'
    if (Test-Path $studioTemplate) {
        $TEMPLATE_FILE = $studioTemplate
    }
}
...
function Validate-Environment {
    ...
    if (-not (Test-Path $TEMPLATE_FILE)) {
        ...
        Write-Info 'Ensure studio/templates/sdd-docs/agent-file-template.md exists, or create it manually.'
        exit 1
    }
}
...
function Update-SpecificAgent {
    ...
    switch ($Type) {
        ...
        default { Write-Err "Unknown agent type '$Type'"; Write-Err 'Expected: claude|gemini|copilot|cursor-agent|qwen|opencode|codex|windsurf|kilocode|auggie|roo|codebuddy|amp|shai|q|qodercli|kiro-cli|kiro|agy|bob|jules'; return $false }
    }
}
